...
class FilterViewAdapter(var mFilterListener: FilterListener) :  RecyclerView.Adapter<FilterViewAdapter.ViewHolder>(){

    private var mPairList: ArrayList<Pair<String, PhotoFilter>> = ArrayList()
    ...
    override fun onBindViewHolder(holder: FilterViewAdapter.ViewHolder, position: Int) {
        ...
        val fromAsset: Bitmap? = getBitmapFromAsset(holder.itemView.context, filterPair.first)
    ...
    override fun getItemCount()= mPairList.size

    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView){

        var mImageFilterView = itemView.findViewById<ImageView>(R.id.imgFilterView)
        var mTxtFilterName = itemView.findViewById<TextView>(R.id.txtFilterName)
        init {
            itemView.setOnClickListener {
                mFilterListener.onFilterSelected(mPairList.get(layoutPosition).second)
            }
        }
    }
