...
class MeetNewOpenMeetAdapter(var myContext: Activity,
                             var fragment: MeetUpNew,
                             var list: GetMeetUpResponse?,
                             var toolTip : ()-> Unit) :
    RecyclerView.Adapter<MeetNewOpenMeetAdapter.CardViewHolder>()  {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) =
        CardViewHolder(
            LayoutInflater.from(parent.context).inflate(R.layout.card_core_open_meet, parent, false),myContext,viewPool
        )

    override fun onBindViewHolder(holder: CardViewHolder, position: Int) {
        var data = list?.get(position)
        Log.i(TAG, " checkingPreviousdata:   ${data}")
        if((data?.join_requests?.requests?.size?.compareTo(0) == 1)&&position==0 && !isShown && fragment.rootCo!=null){
            isShown=true
            toolTip()
        }
        data?.name?.let {
        ...
        if(data?.status.equals(MeetStatus.CANCELLED.status)){
            ...
            holder.rlCancel.visibility = View.INVISIBLE
        ...
        if(data?.join_requests?.requests?.size?.compareTo(0) == 1){
            ...
            holder.rvIntrested.adapter = OpenMeetJoinRqPeopleStackAdapter(myContext,data.toOpemMeet()) {
                MyApplication.putTrackMP(Constant.AcOMInterestedList, JSONObject(mapOf("meetId" to data?._id)))
                if(data.user_id.equals(MyApplication.SID)) {
                    var fragment = RequestToJoinOpenFragment.getInstance(data?._id)
                    Navigation.addFragment(myContext, fragment, RequestToJoinOpenFragment.TAG, R.id.homeFram, true, false)
                }else{
        ...
        holder.view.onClick({
            MyApplication.putTrackMP(Constant.AcOMViewDetail, JSONObject(mapOf("meetId" to data?._id)))
